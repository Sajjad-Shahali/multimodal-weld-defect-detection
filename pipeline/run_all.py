...
Usage:
    ...
    # Logging controls
    python -m pipeline.run_all --steps 1 2 3 4 5 6 --verbose
    python -m pipeline.run_all --steps 1 2 3 4 5 6 --log-level ERROR
"""
...
# Force UTF-8 output on Windows so Unicode characters (─, ✅, →, etc.) work
# in PowerShell / cmd.exe without UnicodeEncodeError.
# Guarded to avoid crashing in environments where sys.std*.buffer is missing.
def _force_utf8_text(stream):
    try:
...
if sys.platform.startswith("win"):
    sys.stdout = _force_utf8_text(sys.stdout)
    sys.stderr = _force_utf8_text(sys.stderr)

# Ensure the project root is on sys.path so the local "pipeline" package
# takes precedence over an identically-named installed package.
...
def main():
    ...
    parser.add_argument(
        ...
        help="Logging level (overrides --verbose). Default: WARNING",
    )
